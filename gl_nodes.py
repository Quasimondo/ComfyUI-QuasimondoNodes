...
class CustomShader:
        ...
        return {"required":
                    {
                     ...
                     "output_size": (["Custom","texture_0","texture_1","texture_2","texture_3"],),
                     "width": ("INT", {"default": 1024, "min": 1, "max": 65536}),
                     "height": ("INT", {"default": 1024, "min": 1, "max": 65536}),
                     "v0": ("FLOAT", {"default": 0.0, "min": -65536.0, "max": 65536.0, "step": 0.001, "round": 0.001}),
                     "v1": ("FLOAT", {"default": 0.0, "min": -65536.0, "max": 65536.0, "step": 0.001, "round": 0.001}),
                     "v2": ("FLOAT", {"default": 0.0, "min": -65536.0, "max": 65536.0, "step": 0.001, "round": 0.001}),
                     "v3": ("FLOAT", {"default": 0.0, "min": -65536.0, "max": 65536.0, "step": 0.001, "round": 0.001}),
                     "v4": ("FLOAT", {"default": 0.0, "min": -65536.0, "max": 65536.0, "step": 0.001, "round": 0.001}),
                     "v5": ("FLOAT", {"default": 0.0, "min": -65536.0, "max": 65536.0, "step": 0.001, "round": 0.001}),                                                                                                         
                     "fragment_code": ("STRING", { "multiline": True, "dynamicPrompts": False, "default": "void main() {\n\tvec2 uv_text = gl_FragCoord.xy/u_resolution.xy;\n\tgl_FragColor = texture(texture_0,uv_text);\n}"}),
                     "sampling": (["LINEAR","NEAREST"],),
                     "border":(["REPEAT/REPEAT","CLAMP/CLAMP","CLAMP/REPEAT","REPEAT/CLAMP"],),
                    },
                "optional":     
                    {
                     "texture_0": ("IMAGE",),
    ...
    def run(self, frames, output_size, width, height, v0,v1,v2,v3,v4,v5,fragment_code, sampling, border, 
            texture_0=None, mask_0=None,
            texture_1=None, mask_1=None,
            texture_2=None, mask_2=None,
            texture_3=None, mask_3=None ):
        ...
        if not texture_0 is None:
            ...
            if not mask_0 is None:
                ...
                t0 = torch.cat((texture_0[0],torch.ones((height0,width0,1),dtype=texture_0.dtype,device=device0)),dim=-1)        
        ...
        if not texture_1 is None:
            ...
            if not mask_1 is None:
                ...
                t1 = torch.cat((texture_1[0],torch.ones((height2,width2,1),dtype=texture_1.dtype,device=device2)),dim=-1)  
        ...
        if not texture_2 is None:
            ...
            if not mask_2 is None:
                ...
                t2 = torch.cat((texture_2[0],torch.ones((height3,width3,1),dtype=texture_2.dtype,device=device3)),dim=-1)       
        ...
        if not texture_3 is None:
            ...
            if not mask_3 is None:
                ...
                t3 = torch.cat((texture_3[0],torch.ones((height4,width4,1),dtype=texture_3.dtype,device=device4)),dim=-1)                                
...
class SpringMesh:
        ...
        return {"required":
                    {
                     ...
                     "offset_zero": ("FLOAT", {"default": 0.0, "min": -1.0, "max": 1.0, "step": 0.001, "round": 0.001}),
                     "offset_scale_x": ("FLOAT", {"default": 0.1, "min": -10.0, "max": 10.0, "step": 0.0001, "round": 0.0001}),
                     "offset_scale_y": ("FLOAT", {"default": 0.1, "min": -10.0, "max": 10.0, "step": 0.0001, "round": 0.0001}),
                     "spring_constant_nb": ("FLOAT", {"default": 0.333, "min": -65536.0, "max": 65536.0, "step": 0.001, "round": 0.001}),
                     "spring_constant_grid": ("FLOAT", {"default": 0.333, "min": -65536.0, "max": 65536.0, "step": 0.001, "round": 0.001}),
                     "time_step": ("FLOAT", {"default": 0.5, "min": 0.000001, "max": 1.0, "step": 0.001, "round": 0.001}),
    ...
    def euler_method(self,points, grid, connections, resting_lengths, spring_constant_nb= 0.1, spring_constant_grid= 0.1,t=50, time_step=0.1):
        velocities = np.zeros_like(points)
        for _ in range(t):
            forces = self.calculate_forces(points, connections, resting_lengths, spring_constant_nb )
            forces = self.calculate_grid_forces(points, grid, forces, spring_constant_grid )
            velocities += forces * time_step
            points += velocities * time_step
        return points

    def run(self, texture, frames, output_size, width, height, cols,rows,offset_sampling, offset_normalize, offset_zero, offset_scale_x,offset_scale_y,
            spring_constant_nb, spring_constant_grid, time_step, t_steps, cumulative, sampling, border, show_vertices, motion_map=None,  mask=None):
        ...
        if not mask is None:
            ...
            t0 = torch.cat((texture[0],torch.ones((height0,width0,1),dtype=texture.dtype,device=device0)),dim=-1)        
        ...
        for row in range(rows):
            ...
            for col in range(cols):
                ...
                if col<cols-1 and row<rows-1:
                    index = row*cols+col
                    render_indices += [index,index+1,index+cols+1,index,index+cols+1,index+cols]
        ...
        resting_lengths = np.linalg.norm(v2d[connections[:,0]] - v2d[connections[:,1]],axis=1)
        ...
        if not motion_map is None:
            ...
            offset_channels = np.array([[0,1,2]["RGB".index(offset_sampling[1])],[0,1,2]["RGB".index(offset_sampling[0])]])
            o_s = np.array([[offset_scale_x/(motion_map.size(1) if offset_normalize else 1.0),offset_scale_y/(motion_map.size(2) if offset_normalize else 1.0)]])
            v2d += o_s * ((motion_map[0,:,:,offset_channels].view(-1,2)[distortion].cpu().numpy()) - offset_zero)
            v2d = self.euler_method(v2d, v2d_t, connections, resting_lengths,spring_constant_nb=spring_constant_nb,spring_constant_grid=spring_constant_grid, t=t_steps, time_step=time_step )
        ...
        vao = ctx.vertex_array(program=prog, content=vao_content, index_buffer=index_buffer)   
        try:
            prog['texture_0'] = 0
            data = list(t0.flatten().float().cpu().numpy().astype(np.float32))
            num_frags = width0 * height0 * 4
        ...
        for i in range(frames):
            ...
            if frames>1:
                if i<motion_map.size(0):
                    ...
                    if not cumulative:
                        v2d =  v2d_t.copy()
                    v2d += o_s * ((motion_map[i,:,:,offset_channels].view(-1,2)[distortion].cpu().numpy()) - offset_zero)
